...
public class RunPersonalizedPageRankBasic extends Configured implements Tool {
    ...
    public void map(IntWritable nid, PageRankNode node, Context context)
      ...
      if (node.getAdjacenyList().size() > 0) {
        ...
        for (int i = 0; i < sources.length; i++){
          mass.add(temp.get(i) - (float) StrictMath.log(list.size())); //***
        }
    ...
    public void reduce(IntWritable nid, Iterable<PageRankNode> iterable, Context context)
      ...
      while (values.hasNext()) {
        ...
        if (n.getType().equals(PageRankNode.Type.Structure)) {
          ...
          ArrayListOfFloatsWritable temp = n.getPageRank();
          for (int i = 0; i < sources.length; i++){
            mass.set(i, sumLogProbs(mass.get(i), temp.get(i))); //***
          }
    ...
    public void map(IntWritable nid, PageRankNode node, Context context)
      ...
      for (int i = 0; i < sources.length; i++){
        if (nid.get() == Integer.parseInt(sources[i])){
          Float temp = Float.parseFloat(missingMass[i]);
          if ( temp <= 0.0f ){
            temp = 0.0f;
          }
          ...
          float link = (float) Math.log(1.0f - ALPHA)
            + sumLogProbs(p.get(i), (float) (Math.log(temp)));
  ...
  private void iteratePageRank(int i, int j, String basePath, int numNodes, String sources) throws Exception {
    ...
    ArrayListOfFloatsWritable mass = phase1(i, j, basePath, numNodes, sources);
  ...
  private ArrayListOfFloatsWritable phase1(int i, int j, String basePath, int numNodes, String sources) throws Exception {
    ...
    int numPartitions = 0;
    for (FileStatus s1 : FileSystem.get(getConf()).listStatus(new Path(in))) {
      if (s1.getPath().getName().contains("part-")) //***
        numPartitions++;
    }
  ...
  private void phase2(int i, int j, ArrayListOfFloatsWritable missing, String basePath, int numNodes, String sources) throws Exception {
    Job job = Job.getInstance(getConf());
    ...
    for (int k = 0; k < missing.size(); k++){
      missing1 += missing.get(k) + ",";
    }
    missing1 = missing1.substring(0, missing1.length()-1);

    job.getConfiguration().setBoolean("mapred.map.tasks.speculative.execution", false);
    job.getConfiguration().setBoolean("mapred.reduce.tasks.speculative.execution", false);
    job.getConfiguration().setStrings("MissingMass", missing1);
    job.getConfiguration().setInt("NodeCount", numNodes);
